...
export default function CustomerMenuPage() {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      router.push(`/menu/order/${res.data.id}`);
    } catch (err: any) {
        ...
        <div className="px-4 pb-3 flex gap-2 overflow-x-auto scrollbar-hide">
          ...
          {categories.map(c => (
            <button
              ...
              className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${activeCategory === c.id ? 'text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              style={activeCategory === c.id ? { backgroundColor: c.color } : undefined}
            >
              {getProductEmoji(c.name, c.name)} {c.name}
            </button>
          ))}
        </div>
      </header>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {filteredProducts.map(p => {
              ...
              const color = p.category?.color || '#6366f1';
              return (
                <div key={p.id} className="card flex flex-col">
                  <div
                    className="h-20 rounded-lg flex items-center justify-center text-3xl mb-2 overflow-hidden"
                    style={{ background: `linear-gradient(135deg, ${color}22, ${color}08)` }}
                  >
                    {p.imageUrl ? (
                      <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover" />
                    ) : (
                      <span>{getProductEmoji(p.name, p.category?.name)}</span>
                    )}
                  </div>
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    {qty === 0 ? (
                      ...
                      <div className="flex items-center gap-2 bg-indigo-50 rounded-lg px-1">
                        <button onClick={() => updateQty(p.id, qty - 1)} className="p-1 text-indigo-600 hover:bg-indigo-100 rounded"><Minus size={14} /></button>
                        <span className="text-sm font-semibold w-4 text-center">{qty}</span>
                        <button onClick={() => updateQty(p.id, qty + 1)} className="p-1 text-indigo-600 hover:bg-indigo-100 rounded"><Plus size={14} /></button>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {cartItems.length === 0 ? (
                ...
                <div key={product.id} className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 bg-gray-100 rounded-lg px-1 shrink-0">
                    <button onClick={() => updateQty(product.id, quantity - 1)} className="p-1 text-gray-600 hover:bg-gray-200 rounded"><Minus size={14} /></button>
                    <span className="text-sm font-semibold w-4 text-center">{quantity}</span>
                    <button onClick={() => updateQty(product.id, quantity + 1)} className="p-1 text-gray-600 hover:bg-gray-200 rounded"><Plus size={14} /></button>
                  </div>
