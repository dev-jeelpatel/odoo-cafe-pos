...
const UNITS = ['piece', 'kg', 'liter'];

export default function ProductsPage() {
  const qc = useQueryClient();
  const [modal, setModal] = useState(false);
  const [editing, setEditing] = useState<Product | null>(null);
  const [form, setForm] = useState({ name: '', category: '', price: '', unit: 'piece', tax: '0', description: '' });
  ...
  const openCreate = () => { setEditing(null); setForm({ name: '', category: '', price: '', unit: 'piece', tax: '0', description: '' }); setModal(true); };
  const openEdit = (p: Product) => { setEditing(p); setForm({ name: p.name, category: p.category._id, price: String(p.price), unit: p.unit, tax: String(p.tax), description: p.description }); setModal(true); };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {products.map(p => (
            <div key={p._id} className="card hover:shadow-md transition-shadow">
              ...
              <p className="text-xs text-gray-500 mt-0.5">{p.category?.name} · {p.unit}</p>
              <div className="flex items-center justify-between mt-3">
                ...
                <div className="flex gap-1">
                  <button onClick={() => openEdit(p)} className="p-1.5 hover:bg-indigo-50 rounded-lg text-indigo-600 transition-colors"><Pencil size={14} /></button>
                  <button onClick={() => del(p._id)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-500 transition-colors"><Trash2 size={14} /></button>
                </div>
              </div>
            </div>
          ))}
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <select value={form.unit} onChange={e => setForm(p => ({ ...p, unit: e.target.value }))} className="input">
              {UNITS.map(u => <option key={u} value={u} className="capitalize">{u.charAt(0).toUpperCase() + u.slice(1)}</option>)}
            </select>
          </div>
          <div>
