            ...
            <tbody className="divide-y divide-gray-50">
              {orders.map(o => (
                <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-500 text-xs">{o.orderType.replace('_', '-').toLowerCase()}</td>
                  <td className="px-4 py-3"><span className={clsx('badge capitalize', statusColors[o.status])}>{o.status.toLowerCase()}</span></td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      {!o.isPaid && o.status !== 'CANCELLED' && <button onClick={() => cancel(o.id)} className="p-1.5 hover:bg-orange-50 rounded-lg text-orange-500"><XCircle size={14} /></button>}
                      {o.status === 'DRAFT' && <button onClick={() => del(o.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-500"><Trash2 size={14} /></button>}
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {selectedOrder && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><span className="text-gray-500">Type:</span> <span className="ml-1">{selectedOrder.orderType.replace('_', '-').toLowerCase()}</span></div>
